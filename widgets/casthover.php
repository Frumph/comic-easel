...
Plugin URI: http://www.cosmichellcats.com/wp/2012/08/8923
...
function insert_character_hovercard($character) {
  $ccard = '';
  if ($character) {
...
function add_characters_hovercards($post_characters) {
  global $post;
...
add_filter('ceo_display_comic_characters', 'add_characters_hovercards');
add_action('wp_enqueue_scripts','casthover_res_init');
add_action('widgets_init', create_function('','return register_widget("casthover_reference_widget");'));


//widget code below here
class casthover_reference_widget extends WP_Widget {
  
  function casthover_reference_widget() {
