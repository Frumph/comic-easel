 ...
 */
function comiceasel_get_calendar($initial = true, $echo = true, $taxonomy = 'post') {
	...
	if ( $previous ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="prev"><a href="' . get_month_link($previous->year, $previous->month) . '?post_type='.$taxonomy.'" title="' . sprintf(__('View posts for %1$s %2$s'), $wp_locale->get_month($previous->month), date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year))) . '">&laquo; ' . $wp_locale->get_month_abbrev($wp_locale->get_month($previous->month)) . '</a></td>';
	} else {
	...
	if ( $next ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="next"><a href="' . get_month_link($next->year, $next->month) . '?post_type='.$taxonomy.'" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s'), $wp_locale->get_month($next->month), date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) ) . '">' . $wp_locale->get_month_abbrev($wp_locale->get_month($next->month)) . ' &raquo;</a></td>';
	} else {
	...
	for ( $day = 1; $day <= $daysinmonth; ++$day ) {
		...
		if ( in_array($day, $daywithpost) ) // any posts today?
				$calendar_output .= '<a href="' . get_day_link($thisyear, $thismonth, $day) . '?post_type='.$taxonomy.'" title="" '. esc_attr($ak_titles_for_day[$day]) . '">'.$day.'</a>';
		else
			$calendar_output .= $day;
		$calendar_output .= '</td>';
 ...
 * @since 2.1.0
 */
function comiceasel_delete_get_calendar_cache() {
	wp_cache_delete( 'get_comic_calendar', 'calendar' );
}
add_action( 'save_post', 'comiceasel_delete_get_calendar_cache' );
add_action( 'delete_post', 'comiceasel_delete_get_calendar_cache' );
add_action( 'update_option_start_of_week', 'comiceasel_delete_get_calendar_cache' );
add_action( 'update_option_gmt_offset', 'comiceasel_delete_get_calendar_cache' );


class comiceasel_calendar_widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if (!empty($instance)) { extract($instance); } ?>
			...
			<div id="wp-calendar-wrap">
				...
				<?php } ?>
			<?php
				comiceasel_get_calendar(true, true, 'comic'); 
			?>
			</div>
